...
from constants import (
    SCROLL_BORDER_X,
    TILE_FIREFLOWER_POINT,
    TILE_CHICKEN_POINT,
    TILE_GOOMBA_POINT,
    TILE_IRONBALL_POINT,
)
from entities import FireFlower, Chicken, Player, Mob
    ...
    def spawn_enemy(self, left_x, right_x):
        ...
        for tx in range(left_x, right_x + 1):
            for ty in range(16):
                ...
                if tile_type == TILE_GOOMBA_POINT:  # クリボー
                    enemies.append(Mob(game, x, y, False))
                elif tile_type == TILE_IRONBALL_POINT:  # 鉄球
                    enemies.append(Mob(game, x, y, True))
                elif tile_type == TILE_CHICKEN_POINT:  # 鶏
                    enemies.append(Chicken(game, x, y))
                elif tile_type == TILE_FIREFLOWER_POINT:  # ファイアーフラワー
                    enemies.append(FireFlower(game, x, y))
                else:
                    continue
